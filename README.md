...
## Product context: teaching spokes about YOUR app

The `product` section in your config teaches every spoke what your app does, what modules it has, how to log in, and what test data exists. Without it, each spoke needs to read multiple files to understand your product. With it, the Control includes everything in the instruction block.

```yaml
product:
  modules:
    - name: "Dashboard"
      description: "Main analytics view with KPIs and charts"
      url: "/dashboard"
      roles: ["admin", "manager"]

  test_accounts:
    - role: "admin"
      login: "/auth/demo-login?role=admin"
      name: "Demo Admin"
      description: "Full access to everything"

  demo_data:
    description: "Generated by scripts/seed_demo.py"
    scale: "50 users, 10 teams, 200 records"
```

**How each spoke uses it:**

| Spoke | What it reads from `product` |
|---|---|
| **QA** | Modules to test per role + login URLs + expected data scale |
| **Journey** | Features to show personas + login as persona's role |
| **Feature** | Existing modules (avoid duplication) + test per role |
| **Dev** | Where to apply fixes + verify per role |
| **Deploy** | URLs to smoke-test + verify login post-deploy |

If `product` is not defined, the skill still works but spokes get less context. For QA and Journey, having it makes a **huge difference**.
...
## Contexto del producto: enseñarle a los spokes sobre TU app

La sección `product` en tu config le enseña a cada spoke qué hace tu app, qué módulos tiene, cómo loguearse, y qué data de test existe.

```yaml
product:
  modules:
    - name: "Dashboard"
      description: "Vista de analytics con KPIs"
      url: "/dashboard"
      roles: ["admin", "manager"]
  ...
  demo_data:
    scale: "50 usuarios, 10 equipos"
```

| Spoke | Qué lee de `product` |
|---|---|
| **QA** | Módulos a testear + login por rol + escala de datos |
| **Journey** | Features a mostrar a las personas + login como cada rol |
| **Feature** | Módulos existentes + test por rol |
| **Dev** | Dónde aplicar fixes + verificar por rol |
| **Deploy** | URLs para smoke-test + login post-deploy |

Si `product` no está definido, el skill funciona pero los spokes tienen menos contexto. Para QA y Journey, tenerlo hace una gran diferencia.
